#ifndef BAIDU_XPU_API_INCLUDE_XPU_REFACTOR_UTIL_FLOAT16_H
#define BAIDU_XPU_API_INCLUDE_XPU_REFACTOR_UTIL_FLOAT16_H
...
#define ALIGN_ATTR
#else
#define ALIGN_ATTR __attribute__((aligned(2)))
#endif
...
inline uint16_t fp16_ieee_from_fp32_value(float f) {
    ...
    uint16_t ret = (sign >> 16) | (shl1_w > UINT32_C(0xFF000000) ? UINT16_C(0x7E00) : nonsign);
    if ((ret & 0x7C00) == 0) { // skip denormal value since XPU doesn't support denormal
        ret = ret & 0x8000;   // keep sign bit
    }
    return ret;
}
#endif

// XXX(miaotianxiang): compilation error in gcc-8.2 if |DLL_EXPORT| and c++11 keyword |alignas| were
// used together...
struct DLL_EXPORT ALIGN_ATTR float16 {
    unsigned short x;
    ...
    constexpr float16(unsigned short bits, from_bits_t) : x(bits) {};
...
#ifdef __F16C__
    x = _cvtss_sh(value, 5); //5 means MXCSR.RC rounding
#else
...
inline float& operator+=(float& a, const float16& b) {
    return a += static_cast<float>(b);
}
inline float& operator-=(float& a, const float16& b) {
    return a -= static_cast<float>(b);
...
inline float16 operator+(float16 a, int64_t b) {
    return a + static_cast<float16>(b);
}
inline float16 operator-(float16 a, int64_t b) {
    return a - static_cast<float16>(b);
...
inline float16 operator+(int64_t a, float16 b) {
    return static_cast<float16>(a) + b;
}
inline float16 operator-(int64_t a, float16 b) {
    return static_cast<float16>(a) - b;
...
public:
    ...
    static constexpr float16 min() {
        return float16(0x0400, float16::from_bits());
    }
    static constexpr float16 lowest() {
        return float16(0xFBFF, float16::from_bits());
    }
    static constexpr float16 max() {
        return float16(0x7BFF, float16::from_bits());
    }
    static constexpr float16 epsilon() {
        return float16(0x1400, float16::from_bits());
    }
    static constexpr float16 round_error() {
        return float16(0x3800, float16::from_bits());
    }
    static constexpr float16 infinity() {
        return float16(0x7C00, float16::from_bits());
    }
    static constexpr float16 quiet_NaN() {
        return float16(0x7E00, float16::from_bits());
    }
    static constexpr float16 signaling_NaN() {
        return float16(0x7D00, float16::from_bits());
    }
    static constexpr float16 denorm_min() {
        return float16(0x0001, float16::from_bits());
    }
};
